...
```{r Imputed Model}
with(sd_imp, lm(ExchNa_ppm ~Date_Sampled +  Year + Irr_season + Depth_1 +  Soil_pH  + NO3_D1_ppm + Bicarb_P_ppm + SO4_ppm + ExchK_ppm + ExchCa_ppm + ExchMg_ppm + Zn_ppm + Mn_ppm + Cu_ppm + Boron_ppm + CEC_me_100g + treatment + ExchK_ppm * treatment)) %>% pool() %>% summary()
```
...
#LASSO for predicting ExchNa 
imp_na <- sd_imp_long %>% select(Date_Sampled, Year, Irr_season, Depth_1, Soil_pH, OM_pct, NO3_D1_ppm, Bicarb_P_ppm, SO4_ppm, ExchK_ppm, ExchCa_ppm, ExchMg_ppm, ExchNa_ppm, Zn_ppm, Fe_ppm, Mn_ppm, Cu_ppm, Boron_ppm, CEC_me_100g, treatment)
...
```{r}
sd_imp_long %>% group_by(.imp) %>%
  ggplot(aes(Date_Sampled, ExchNa_ppm, col=as.factor(Depth_1))) +
    geom_point() +
...
```{r Sodium Tables}
sd %>% group_by(Year) %>% summarise("mean sodium"=mean(ExchNa_ppm))
sd %>% group_by(Depth_1) %>% summarise(Sodium=mean(ExchNa_ppm))

sd %>% group_by(Depth_1) %>% summarise(Sodium=mean(ExchNa_ppm)) %>% ggplot(aes(x=Depth_1, y=Sodium)) + geom_bar(stat="identity") + geom_label(aes(label=Sodium)) + theme_bw()
```
